...
//             className="text-xl"
//           >
//           <img src={assets.back_icon} className="w-5" alt="" />
...
const Navbar = () => {
  ...
  return (
    <div className="flex items-center justify-between py-5 font-medium relative">
        ...
        {["/", "/collection", "/about", "/contact"].map((path, i) => {
          ...
          return (
            <NavLink
              key={path}
              className={({ isActive }) =>
                `flex flex-col items-center gap-1 p-3 ${isActive ? "active-link" : ""
                }`
              }
              to={path}
            >
      ...
      {/* Sidebar Menu */}
      <div
        className={`fixed top-0 left-0 h-full w-64 bg-white shadow-lg transform sm:hidden ${isMenuOpen ? "translate-x-0" : "-translate-x-full"
          } transition-transform duration-300 z-50`}
      >
        {/* Close Button */}
        <div className="flex justify-between items-center p-4 border-b">
          ...
          {["/", "/collection", "/about", "/contact"].map((path, i) => {
            ...
            return (
              <NavLink
                key={path}
                className={({ isActive }) =>
                  `px-6 py-4 border-b ${isActive ? "bg-gray-100 font-semibold" : "hover:bg-gray-50"
                  }`
                }
                to={path}
                onClick={() => setIsMenuOpen(false)}
